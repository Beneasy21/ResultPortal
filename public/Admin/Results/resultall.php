	...
	<?php 
		...
		while($rowStudent = mysqli_fetch_array($student))
			...
			$RegNo =$rowStudent["studId"];
				$rowStudent = fetch_a_student_for_batch($RegNo, $Session);
		...
		<tr>
			<td>
				...
				<?php
					...
					$studentResult = fetch_individual_result($RegNo,$Session, $Term, $Armm);
					while($rowResult = mysqli_fetch_array($studentResult))
					{
					$Subject = $rowResult['subName'];
					$CA1=$rowResult['ca1'];
					$CA2=$rowResult['ca2'];
					$Exams=$rowResult['exam'];
					$Total= $CA1 + $CA2 +$Exams;
					$HIC=$rowResult['chm'];
					$LIC=$rowResult['clm'];
					$AIC=$rowResult['cam'];
